            ...
            if (captcha == ''):
                if (count < 6):
                    ...
                    raise IncorrectCaptcha(
                        "Invalid Captcha.")
            password_url = driver.current_url
            driver.find_element(
                By.ID, 'txtLoginPassword').send_keys(self._password)

            driver.find_element(By.ID, 'txtcaptcha').send_keys(captcha)
            time.sleep(0.5)

            if (check_by_xpath('//*[@id="login-page"]/div/div[2]/p')):
                ...
                elif (error == 'User Id or Password In Correct'):
